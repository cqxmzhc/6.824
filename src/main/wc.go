...
func Map(value string) *list.List {
    getWords := func(c rune) bool {
        return !unicode.IsLetter(c)
    }

    validWords := strings.FieldsFunc(value, getWords)
    ...
    for key, value := range tmp {
        mapList.PushBack(mapreduce.KeyValue{key, strconv.Itoa(value)})
    }
...
func Reduce(key string, values *list.List) string {
    ...
    for e := values.Front(); e != nil; e = e.Next() {
        if num, err := strconv.Atoi(e.Value.(string)); err == nil {
            res += num
        }
    }
